...
This package consolidates common questions and provides the data in a structure that is much easier to work with and interpret, since some of the basic factor recoding has already been done. We also calculate alternative sample weights, based on demonstrated
changes in non-response bias over the decades, and recommended as best practices by some
elections analysts (documentation of these materials is provided in [the voting vignette](voting.html)).
...
```{r eval = FALSE}
cps <- cps_load_basic()  # Load All Years
                         # May take some time to download and
                         #  process files the first time! 
```

```{r eval = FALSE}
cps <- cps_load_basic(years = c(2006, 2008)) # Just load 2006 and 2008
```
...
These estimated follow closely Dr. Michael McDonald's [estimates of turnout](http://www.electproject.org/2018g), among eligible voters in the November 2018 General Election. For a detailed examination of how non-response bias has affected
the use of CPS for estimating turnout, see the [voting vignette](voting.html). We thank the U.S. Elections Project at the University of Florida for the turnout estimates.  
...
`cps_load_basic()` is a wrapper for several constituent steps that have their own parameters and assumptions. We've detailed the changes made to get from the raw data file to the cleaned file in the [add variables](addvariables.html) vignette.
...
- `cps_recode_vote()` recodes the variable `VRS_VOTE` according to two different assessments of voter turnout. The new variable `cps_turnout` will calculate turnout the same way that the census does, while another new variable `hurachen_turnout` will calculate turnout according to Hur and Achen (2013). These two methods differ in how they count responses of "Don't know", "Refused", and "No response"; see the BLANK vignette for more details.
- `cps_reweight_turnout()` adds a new variable, `turnout_weight`, that reweights the original `WEIGHT` to account for the adjusted turnout measure. This corrects for increased nonresponse to the VRS over time, as well as a general pattern of respondents overreporting their personal voting history (though the CPS sees less overreporting than other surveys). See the BLANK vignette for details.
