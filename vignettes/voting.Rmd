---
title: "Using the CPS To Understand Voting"
...
these non-response categories as "not voted"--essentially added these
...
## Refactor variables that differ across years
...
cps2016_srvyrHW %>%
  ...
  scale_x_discrete(labels = c("WHITE"= "White",
                                "BLACK" = "Black", 
                                "AMERICAN INDIAN, ALASKAN NATIVE" = "Amer Indian/Alakan",
                                "ASIAN" = "Asian", 
                                "HAWAIIAN/PACIFIC ISLANDER" = "Hawaii/PacIsland")) +
...
Simple map of voter turnout by state. 
...
Introductory text that explains the different coding that was used
over time by the CPS. 

Recoding of Vote Mode is completed in `cps_label` step. Explain what
is done here.

Canyon Tables here that show the different coding rules and the 
`cpsvote` final variable, `VRS_VOTEMETHOD_CON`
...
cps10k_weight %>%
   mutate(census_region = factor(census_region),
         VRS_VOTEMETHOD_CON = factor(VRS_VOTEMETHOD_CON)) %>%
  filter(YEAR > 1994 &  !is.na(VRS_VOTEMETHOD_CON)) %>%
