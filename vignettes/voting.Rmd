...
Census provided weight (labeled `WEIGHT` after the renaming and recoding process in `cps_vote`).

## <a name="sample2016"></a>Sample Usage: Estimating Voter Turnout in 2016

Start by reading the files, performing specialized `cpsvote` recoding, and creating new survey weights. This same example appears in the [background vignette](background.html), but in that vignette, the simpler `cps::cps_load_basic` syntax is used.
