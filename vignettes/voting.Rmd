...
## Voting: How the `cpsvote` package makes it easier to use the CPS Voting and Registration Supplement to examine voting

The Current Population Survey's Voting and Registration Supplement, collected in November of every
federal election year, is an important source to understand many aspects of the American elections system.
Because of its 
...
and in individual states, regions, and among key demographic groups. The CPS is one of the primary ways we
understand voter registration, 
voting turnout, and use of different modes of voting (Election Day, Early In Person, Voting By Mail) in the
United States since 1996.

This vignette focuses on how the `cpsvote` package provides a 
simple avenue to examine voting turnout and mode of voting. The vignette includes detailed documentation of
how the CPS codes voting turnout and mode of voting, and 
...
to adjust for this bias. Finally, the vignetee includes a series of examples of tables and visualizations of voting
turnout and use of different modes of voting that illustrate use of `cpsvote`. 

The vignette is organized into the following sections:

1. [Overview](#overview) of challenges with using the CPS to study voter turnout
2. [Voting Turnout](#votingturnout): Review of the problem
3. [Sample Usage](#sample2016): Estimating Voter Turnout In 2016
4. [Example](#race2016): Turnout by racial groups
5. [Example](#map2016): A simple map of voter turnout by state
6. [Voting Mode in the CPS](#votingmode): Election Day, Early In Person, and Absentee/Voting By Mail
7. [Example](#modegrowth): Growth in Early In Person and Vote By Mail, 1996-2018
8. [Example](#vbmgrowth): Regional Use of Voting at Home, 1996-2018
9. [Example](#modemaps): Maps of By Mail, Early In Person, and Election Day Voting in 2016

## <a name="overview"></a>Overview of how `cpsvote` helps resolve challgnes with using the CPS to study voting turnout
...
## <a name="votingturnout"></a>Voting turnout using the CPS: review of the problem
...
Professor Michael McDonald of the University of Florida helpfully provides guidance on this more complex procedure. He provides <Commentary, Guidelines, and Stata Code>[link](http://www.electproject.org/home/voter-turnout/cps-methodology) at his website. We have adapted this code for `R` and have integrated
it into the `cpsvote` package. 
...
variables (`cps_turnout` and `hurachen_turnout`) and produce a Hur-Achen weight (`turnout_weight`). 
...
## <a name="sample2016"></a>Sample Usage: Estimating Voter Turnout in 2016

Start by reading the files and performing specialized `cpsvote` recoding, create new survey weights. This same example appears in the [background vignette](background.html), but uses the simpler `cps::cps_load_basic` syntax.

```{r setup2, echo = TRUE}
#library(cps)
# Download the 2016 CPS, if not already downloaded
# cpsvote::cps_download_data(years = 2016) 

# Read in CPS 2016
cps2016 <- cpsvote::cps_read(years = 2016,
...
### <a name="race2016"></a>Generate Hur/Achen weighted turnout estimates by racial groups
...
### <a name="map2016"></a>A simple map of voter turnout by state

We use the `usmap` package to provide a quick look at voter turnout
...
## <a name="votingmode"></a>Voting Mode in CPS: Election Day, Early In Person, and Absentee/Voting by Mail
...
of Election Day, Early In Person, and Absentee / Voting By Mail across states and over years. 


### <a name="modegrowth"></a> Example: Growth in Alternative Voting Modes

The use of early in-person and voting by mail has grown enormously in the past quarter century, with distinct regional patterns to the use
...
the survey weights to the 10,000 random sample of the CPS.
...
### <a name="vbmgrowth"></a> Example: Growth in Vote By Mail Concentrated in the West

Absentee / Voting By Mail has been most popular among voters in the Western region. This distinct regional pattern is displayed
in the next visualization. By the 2016 election, 70% of ballots were cast by mail, compared to under 20% in the rest of the country. 

```{r voteathome, eval = TRUE}
cps10k_weight %>%
  ...
  labs(title = "Regional Use of Vote At Home, 1996 - 2018", 
       ...
       color = "Region") +
  theme(plot.title = element_text(size = 20, family = "Times", face = "bold.italic", colour = "red"),
        legend.position = c(.1,.7), legend.background = element_rect(),  
...
### <a name="modemaps"></a> Example: Mapping alternative methods of voting
...
areas of the Midwest and South); Early In-Person voting (most popular in Nevada, New Mexico, Texas, Arkansas, Tennessee, North Carolina, and Georgia); and
Absentee / Voting By Mail (most popular in the full vote by mail states -- Colorado, Oregon, and Washington -- as well as California, Arizona, Montana, 
and Hawaii). 
...
sf::st_as_sf(map_data) %>% 
  ...
  ggthemes::theme_map() + 
  theme(legend.position = "top") +
  scale_fill_gradient(low = "yellow", high = "red", na.value = NA, name = "Early In Person Voting Usage", labels = scales::percent)
...
sf::st_as_sf(map_data) %>% 
  ...
  ggthemes::theme_map() + 
  theme(legend.position = "top") + 
  scale_fill_gradient(low = "sky blue", high = "navy", na.value = NA, name = "Absentee / Vote-by-Mail Usage", labels = scales::percent)
