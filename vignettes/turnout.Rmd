...
cps2016 <- cps2016 %>%
  mutate(cps_turnout_d = case_when(
    cps_turnout == "YES" ~ 1,
    cps_turnout == "NO" ~0),
    achenhur_turnout_d = case_when(
    achenhur_turnout == "YES" ~ 1,
    achenhur_turnout == "NO" ~ 0)
  )
...
turnout_estimates <- cps2016 %>%
  summarize(Unweighted = mean(cps_turnout_d, na.rm = TRUE),
            ...
            Hur_Achen_Weighted = svymean(achenhur_turnout_d, cps2016_surveyHW, na.rm = TRUE))
...
cps2016_srvyrHW %>%
  ...
  group_by(RACE) %>%
  summarize(turnout = survey_mean(achenhur_turnout_d, na.rm = TRUE)) %>%
