...
cps2016 <- cpsvote::cps_load_basic(years = 2016, here::here('cps_data')) # Read in CPS 2016. 
...
method, because it fails to account for growing non-response bias. The Hur-Achen weighted estimate of 2016 turnout,  `r turnout_estimates$Hur_Achen_Weighted`, is closest to the 60.1\% estimate
provided by the [US Elections Project](http://www.electproject.org/2016g).

## More Resources

See `vignette("basics")` for more information on using `cpsvote`, and further examples of how to work with this data. See `vignette("voting")` for more information and further examples of using the CPS for calculating voter turnout and other voting-related quantities. 
