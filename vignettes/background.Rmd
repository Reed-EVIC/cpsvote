...
## Background: Why We Created _`cpsvote`  A Social Science Toolbox For Using the Current Population Survey's Voting and Registration Supplement_
...
challenging to use, especially for less experienced analysts. 

The `cpsvote` package is designed to overcome these challenges. 

The `cpsvote` package eases access to the CPS by completing the following
steps using a single or a small number of commands:
...
More details on these procedures are provided below and in the package
vignettes and README file on github. 
