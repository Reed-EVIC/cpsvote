...
```{r load-data, echo = TRUE, eval = TRUE, cache = TRUE}

# WARNING: THIS NEXT COMMAND DOWNLOADS A LOT OF DATA
#cpsvote::cps_download_data(year = seq(1994,2018,2))
cps <- cps_read(dir = "~/Google Drive/EVIC_Work/Code/gronke_github/CPSVote/cps_data", year = seq(1994, 2016, 2))
...
```{r data-descriptives, echo = TRUE, eval = TRUE}
...
```{r recode-turnout, echo = TRUE, eval = TRUE}
...
cps %>%
  ...
  cps_label() %>%          # Convert columns with factor labels
  cps_recode_vote() %>%    # Create two new vote turnout variables to correspond to CPS and Hur-Achen coding
  tabyl(VRS_VOTE, hurachen_turnout)
...
```{r vote-modes, echo = TRUE, eval = TRUE}
cps_recoded <- cps %>% 
  cps_label() %>%          # Convert columns with factor labels
