# read the file in #####
cpsvrs2012_direct <- readr::read_fwf(here::here('data', 'data2012.txt'),
...
unique(cpsvrs2012_direct$CPS_YEAR) == 2012

# everybody who is OOU for VOTE is OOU for all VRS Qs
not_in_univ <- filter(cpsvrs2012_direct, VRS_VOTE == -1) %>%
  select(starts_with("VRS_"))
all(not_in_univ == -1)

# set the factors #####
cpsvrs2012 <- cpsvrs2012_direct %>%
  ...
  mutate(CPS_STATE = factor(CPS_STATE, levels = fips$state_code, labels = fips$state),
         ...
         VRS_REGHOW = factor(VRS_REGHOW, levels = c(1:9, -1, -2, -3, -9),
                             labels = c("At a department of motor vehicles (for example, when obtaining a driver's license or other identification card)",
                                        ...
                                        "Refused",
                                        "No Response")),
         VRS_RESIDENCE_ORIG = factor(VRS_RESIDENCE_ORIG, levels = c(1:6, -1, -2, -3, -9),
         ...
         VRS_RESIDENCE_RECODE = forcats::fct_recode(VRS_RESIDENCE_ORIG,
                                                    `Less than 1 year` = "Less than 1 month",
                                                    `Less than 1 year` = "1-6 months",
